...
__all__ = ['Classifier', 'ClassifierBCNN']
...
class ClassifierBCNN(nn.Module):

    def __init__(self, backbone: nn.Module, num_classes: int, bottleneck: Optional[nn.Module] = None,
                 bottleneck_dim: Optional[int] = -1, head: Optional[nn.Module] = None, finetune=True):
        super(ClassifierBCNN, self).__init__()
        self.backbone = backbone
        self.num_classes = num_classes
        ...
        if bottleneck is None:
            ...
            self.bottleneck1 = copy.deepcopy(bottleneck)
            self.bottleneck2 = copy.deepcopy(bottleneck)
            assert bottleneck_dim > 0
            self._features_dim = bottleneck_dim
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        f, f1, f2 = self.backbone(x)
        f = self.bottleneck(f)
        f1 = self.bottleneck1(f1)
        f2 = self.bottleneck2(f2)
        predictions = self.head(f)
        return predictions, f, f1, f2
    ...
    def get_parameters(self, base_lr=1.0) -> List[Dict]:
        ...
        params = [
            {"params": self.backbone.parameters(), "lr": 0.1 * base_lr if self.finetune else 1.0 * base_lr},
            {"params": self.bottleneck.parameters(), "lr": 1.0 * base_lr},
            {"params": self.bottleneck1.parameters(), "lr": 1.0 * base_lr},
